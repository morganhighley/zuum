		...
		IConnection3, ITcp
	{
		...
		public void Initialize(IPAddress ipAddress, int port)
		{
			try
			{
				...
				_tcpTransport.Initialize(ipAddress, port);
				ConnectionTransport = _tcpTransport;

				_tcpTransport.DriverID = DriverID;
            ...
            catch (Exception ex)
            {
                if (EnableLogging)
                {
                    Log(string.Format("SecuritySystemProtocol Error: {0}", ex.Message));
                }
            }
        }
		...
		/// <summary>
		/// This is the handler for system state changed event like area info and connectin
		/// This method will listen the changes from protocol and update the system as per changed object
		/// </summary>
		/// <param name="securitySystemStateObjects"></param>
